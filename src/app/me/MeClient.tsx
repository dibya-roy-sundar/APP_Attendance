 ...
 */
/**
 * Every terminal state needs a way onward.
 *
 * Installed to the home screen the app runs without an address bar, so a card
 * with no links leaves the only exit as force-quitting it.
 */
function WayOut({ showAdmin = true }: { showAdmin?: boolean }) {
  return (
    <div className="mt-5 flex flex-wrap justify-center gap-2">
      {showAdmin && (
        <Link
          href="/admin"
          className="inline-flex min-h-11 items-center rounded-xl border border-slate-300 px-4 text-sm font-medium dark:border-slate-700"
        >
          I am the admin
        </Link>
      )}
