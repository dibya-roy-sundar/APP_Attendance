...
const MESSAGES: Record<string, string> = {
  ...
  BAD_DEVICE: 'Your browser blocked local storage. Turn off private browsing and retry.',
...
export function ScanClient({ sessionId, token }: { sessionId: string; token: string }) {
  ...
  const mark = useCallback(async () => {
    ...
    try {
      ...
      if (body.status === 'MARKED') {
        setResult({ kind: 'marked', name: body.name, classDate: body.classDate })
      ...
      } else if (body.error === 'BAD_TOKEN' || body.error === 'SESSION_CLOSED') {
  ...
  async function submitEnroll(e: React.FormEvent) {
    ...
    try {
      ...
      if (body.status === 'ENROLLED' || body.status === 'MARKED') {
        ...
        setEnrollError(MESSAGES[body.error] ?? 'Could not register. Try again.')
      }
    } catch {
  ...
  return (
    <main className="mx-auto flex min-h-dvh max-w-md flex-col justify-center p-6">
      {result.kind === 'loading' && (
        <Card>
          <p className="text-slate-500 dark:text-slate-400">Marking you present…</p>
        </Card>
      )}
      ...
      {result.kind === 'marked' && (
        <Card tone="ok">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">{formatDate(result.classDate)}</p>
          <Link href="/me" className="mt-5 inline-flex min-h-11 items-center justify-center text-sm underline">
      ...
      {result.kind === 'enroll' && (
        <Card>
          ...
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
            Enter your roll number. It gets linked to this phone, and you will not
      ...
      {result.kind === 'offline' && (
        <Card tone="warn">
          ...
          <button
            ...
            className="mt-5 rounded-xl border border-slate-300 px-4 py-2 text-sm dark:border-slate-700"
          >
            Retry
          </button>
        </Card>
      ...
      {result.kind === 'error' && (
        <Card tone="warn">
          ...
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">{result.message}</p>
        </Card>
      )}
    </main>
  )
...
function Card({
  ...
  return (
    <div className={`rounded-2xl p-6 text-center ring-1 ${ring}`}>{children}</div>
  )
}
