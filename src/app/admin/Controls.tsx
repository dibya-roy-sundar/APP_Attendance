...
type Props = {
  ...
  onStart: (opts: { classDate?: string; durationMinutes: number; windowSeconds: number }) => void
  onShowQr: () => void
  onSetOpen: (open: boolean, opts?: { minutes?: number; windowSeconds?: number }) => void
            ...
            <button
              onClick={() => {
                ...
                if (todaySession && session?.classDate === today) {
                  onSetOpen(true, { minutes: duration, windowSeconds: rotation })
                } else {
                  onStart({ durationMinutes: duration, windowSeconds: rotation })
                }
              }}
              disabled={busy}
              className="rounded-xl bg-slate-900 px-4 py-2.5 font-medium text-white disabled:opacity-40 dark:bg-white dark:text-slate-900"
